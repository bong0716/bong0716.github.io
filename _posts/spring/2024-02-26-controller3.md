...
```
그렇다면 해결 방법은? static 폴더에서 templates 폴더로 경로를 이동 시키고 컨트롤러의 return문에서 확장자를 지우면 된다. 확장자를 지워도 잘 찾아가는 이유는 spring에서 mustache를 지원하기 때문. 
...
<p align="center">
컨트롤러까지 생성 후 실행시키면 Whitelabel Error Page가 뜸. return "c"; 만으로는 jsp 파일을 찾을 수 없기 떄문이다. 이는 스프링 부트가 jsp를 지원하지 않기 때문이므로 별도의 설정이 필요하다. 
</p>
